...
export default function PVideos() {
  ...
  async function doSearch() {
    ...
    try {
      let res;
      const urlMatch = q.match(/(?:v=|youtu\.be\/|embed\/)([\w-]{11})/);
      const bareId = /^[\w-]{11}$/.test(q);
      if (urlMatch || bareId) {
        const v = await fetchVideoById(q);
        res = [v];
      } else {
        res = await searchVideos(q);
      }
      setResults(res);
      if (!res.length) setSearchErr('No videos found. Try different keywords.');
    } catch (e) { setSearchErr(e.message || 'Search failed'); setResults([]); }
    finally { setSearching(false); }
  }

  function pinVideo(v) {
    if (pinned.find(x => x.id === v.id)) return;
    d({ t: 'ADD_PIN', v });
    setPinnedNow(prev => ({ ...prev, [v.id]: true }));
  }
  ...
  return (
    <div>
      ...
      <p className="text-[#6b7280] text-sm mb-5">
        Search for videos to pin for your kids — they appear in a special row on the home screen, bypassing all filters.
      </p>
      ...
      <div className="bg-[#1f2937] rounded-xl p-4 mb-6">
        ...
        {results && results.length > 0 && (
          <div className="flex flex-col gap-2 mt-2">
            {results.map(v => {
              const alreadyPinned = !!pinned.find(x => x.id === v.id) || pinnedNow[v.id];
              return (
                <div key={v.id} className="bg-[#111827] border border-[#374151] rounded-xl p-3 flex gap-3 items-center">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[#6b7280] text-xs mt-0.5">{v.chName}</p>
                    <p className="text-[#6b7280] text-xs">{v.views} · {v.date}</p>
                    {v.short && <span className="inline-block mt-1 text-[10px] bg-orange-900 text-orange-300 px-1.5 py-0.5 rounded-full">Short</span>}
                  </div>
                  <div className="flex-shrink-0">
                    {alreadyPinned
                      ? <span className="flex items-center gap-1 text-yellow-400 text-xs px-2 py-1"><IcStar size={12} />Pinned</span>
                      : <button onClick={() => pinVideo(v)}
      ...
      {!pinned.length
        ? (
          ...
          <div className="flex flex-col gap-2">
            {pinned.map(v => (
              <div key={v.id} className="bg-[#1f2937] rounded-xl p-3 flex gap-3 items-center">
                <div className="flex-shrink-0 w-24 sm:w-32">
                  <div className="thumb rounded-lg">
                    <img src={v.thumb} alt={v.title} onError={e => { e.target.style.display = 'none'; }} />
                    <span className="dur">{v.dur}</span>
                  </div>
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-white font-medium text-sm clamp2">{v.title}</p>
                  <p className="text-[#6b7280] text-xs mt-0.5">{v.chName}</p>
                  <p className="text-[#6b7280] text-xs">{v.views} · {v.date}</p>
                  <a href={`https://www.youtube.com/watch?v=${v.yt}`} target="_blank" rel="noopener noreferrer"
                    className="inline-flex items-center gap-1 text-blue-400 text-xs mt-1 hover:text-blue-300">
                    <IcLink size={10} />View on YouTube
                  </a>
                </div>
                <button onClick={() => d({ t: 'DEL_PIN', id: v.id })}
      ...
      <div className="mt-5 bg-[#1e3a5f] border border-blue-800 rounded-xl p-4 text-xs text-blue-400">
        ...
        Pinned videos bypass all filters and appear in a "Parent's Picks" row on the kids' home screen — even if the channel isn't whitelisted.
      </div>
    </div>
  );
}
