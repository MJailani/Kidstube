...
export default function PDash() {
  ...
  const stats = [
    { l: 'Whitelisted Channels', v: whitelistedChannels.length, bg: 'bg-blue-600', to: '/parent/channels' },
    { l: 'Available Videos', v: availableCount, bg: 'bg-green-600', to: '/parent/channels' },
    { l: 'Videos Filtered', v: blockedCount + hiddenShortsCount, bg: 'bg-orange-500', to: '/parent/filters' },
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-5">
        {stats.map((stat) => (
          <button key={stat.l} onClick={() => navigate(stat.to)}
            ...
            <div className={`w-10 h-10 rounded-lg ${stat.bg} flex items-center justify-center flex-shrink-0`}>
              <span className="text-white text-lg font-bold">{stat.v}</span>
            </div>
            <p className="text-[#9ca3af] text-xs leading-tight">{stat.l}</p>
          </button>
        ))}
      </div>
      ...
      <div className="bg-[#1f2937] rounded-xl p-4 mb-4">
        ...
        <div className="flex flex-wrap gap-2">
          {s.filters.blockShorts && <span className="bg-green-900 text-green-300 text-xs px-2 py-1 rounded-full">Shorts blocked</span>}
          <span className="bg-blue-900 text-blue-300 text-xs px-2 py-1 rounded-full">Min {s.filters.minSecs}s</span>
          {s.filters.keywords.map((keyword) => <span key={keyword} className="bg-red-900 text-red-300 text-xs px-2 py-1 rounded-full">"{keyword}"</span>)}
        </div>
      </div>
      ...
      <div className="bg-[#1f2937] rounded-xl p-4">
        ...
        {!s.history.length
          ...
          : s.history.slice(0, 5).map((entry, index) => (
            <div key={index} className="flex items-center gap-3 py-2 border-b border-[#374151] last:border-0">
              <img src={entry.thumb} alt="" className="w-16 h-9 rounded-lg object-cover bg-[#374151] flex-shrink-0"
