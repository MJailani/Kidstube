...
export default function PChannels() {
  ...
  function addChannel(channel) {
    if (allChannels.find((entry) => entry.id === channel.id)) return;
  ...
  return (
    <div>
                  ...
                  <div key={channel.id} className="bg-[#111827] border border-[#374151] rounded-xl p-3 flex items-center gap-3">
                    ...
                    <div className="flex-shrink-0">
                      {alreadyHave || justAdded
                        ...
                        : <button onClick={() => addChannel(channel)}
      ...
      <div className="relative mb-4">
        ...
        <input value={filterQ} onChange={(event) => setFilterQ(event.target.value)} placeholder="Filter your channels..."
        ...
        {list.map((channel) => {
          ...
          return (
            <div key={channel.id} className={`flex items-center gap-3 p-3 rounded-xl border transition-all ${enabled ? 'bg-[#1f2937] border-blue-700' : 'bg-[#111827] border-[#374151] opacity-60'}`}>
              <div onClick={() => d({ t: 'TOGGLE_CH', id: channel.id })} className="flex items-center gap-3 flex-1 min-w-0 cursor-pointer">
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                {!channel.builtin && (
                  <button onClick={() => d({ t: 'DEL_CH', id: channel.id })}
                    className="p-1.5 text-[#6b7280] hover:text-red-400 hover:bg-[#374151] rounded-lg transition-colors" title="Remove channel">
                    <IcTrash size={14} />
                  </button>
                )}
                <div onClick={() => d({ t: 'TOGGLE_CH', id: channel.id })} className={`w-10 h-6 rounded-full transition-colors cursor-pointer ${enabled ? 'bg-blue-600' : 'bg-[#374151]'}`}>
