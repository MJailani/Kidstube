...
export default function PHistory() {
  const { s, clearWatchHistoryForProfile, activeProfile, hasSupabaseAuth } = useApp();
  const [conf, setConf] = useState(false);
  const grouped = {};

  for (const entry of s.history) {
    const dt = new Date(entry.at).toLocaleDateString([], { weekday: 'long', month: 'long', day: 'numeric' });
    grouped[dt] = grouped[dt] || [];
    grouped[dt].push(entry);
  }
    ...
    <div>
        ...
        {s.history.length > 0 && (
          <button
            onClick={async () => {
              ...
              await clearWatchHistoryForProfile();
              setConf(false);
            }}
      ...
      {hasSupabaseAuth && activeProfile && (
        <p className="text-[#6b7280] text-xs mb-4">
          Showing watch history for <span className="text-white">{activeProfile.name}</span>
        </p>
      )}
          ...
          <div key={date} className="mb-5">
            ...
            <div className="bg-[#1f2937] rounded-xl overflow-hidden divide-y divide-[#374151]">
              {entries.map((entry, index) => (
                <div key={index} className="flex items-center gap-3 px-4 py-3 hover:bg-[#374151] transition-colors">
                  <img
                    src={entry.thumb}
                    alt=""
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-white text-sm font-medium clamp1">{entry.title}</p>
                    <p className="text-[#6b7280] text-xs">{entry.chName} | {entry.dur}</p>
                  </div>
