...
import { IcClock, IcFilter, IcGrid, IcList, IcLogOut, IcMsg, IcStar, IcTv } from '../icons';
      ...
      <div className="flex flex-1 overflow-hidden">
        <nav className="w-48 bg-[#1f2937] border-r border-[#374151] flex-col py-4 gap-1 px-2 hidden sm:flex">
          {P_NAV.map((navItem) => (
            <button
              key={navItem.id}
              onClick={() => navigate(`/parent/${navItem.id}`)}
              className={`relative flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium w-full transition-colors ${sec === navItem.id ? 'bg-blue-600 text-white' : 'text-[#9ca3af] hover:bg-[#374151] hover:text-white'}`}
            >
              <navItem.Ic size={16} />
              {navItem.label}
              {navItem.badge && pending > 0 && (
        ...
        <div className="sm:hidden fixed bottom-0 left-0 right-0 bg-[#1f2937] border-t border-[#374151] flex z-50">
          {P_NAV.map((navItem) => (
            <button
              key={navItem.id}
              onClick={() => navigate(`/parent/${navItem.id}`)}
              className={`relative flex-1 flex flex-col items-center gap-0.5 py-2 text-[9px] font-medium ${sec === navItem.id ? 'text-blue-400' : 'text-[#6b7280]'}`}
            >
              <navItem.Ic size={16} />
